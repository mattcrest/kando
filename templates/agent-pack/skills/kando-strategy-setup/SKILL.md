...
**Concepts:** `.kando/kando-for-agents.md` · **Template:** `.kando/templates/initiative.md`

## Prerequisites

1. **kando-roadmap-router** — resolve vault + conventions.
2. Fetch the write contract:
   - **Online:** `GET /api/vaults/<vaultKey>/card-contract`
   - **Offline:** `.kando/card-contract.json`
3. Vault health (when API available): `GET /api/vaults/<vaultKey>/doctor`
...
- Doctor report (if API available): fix **contract errors** before claiming Strategy is set up
...
1. Copy frontmatter from **card-contract** templates or an existing card in the vault.
...
6. **Validate** before and after every file (see release-card-writing).
7. Run doctor again (if API available) and report results to the user.
